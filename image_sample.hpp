...
struct SimplePatchFeature
{
    ...
    void generate(int patch_size, boost::mt19937* rng, int num_feature_channels = 0, float max_sub_patch_ratio = 1.0)
    {
        ...
        boost::uniform_int<> dist_x(0 - patch_size/2, size - rectA.width - 1 - patch_size/2);
        boost::variate_generator<boost::mt19937&, boost::uniform_int<> > rand_x(*rng, dist_x);
        rectA.x = rand_x();

        boost::uniform_int<> dist_y(0 - patch_size/2, size - rectA.height - 1 - patch_size/2);
        boost::variate_generator<boost::mt19937&, boost::uniform_int<> > rand_y(*rng, dist_y);
        rectA.y = rand_y();

        boost::uniform_int<> dist_x_b(0 - patch_size/2, size - rectB.width - 1 - patch_size/2);
        boost::variate_generator<boost::mt19937&, boost::uniform_int<> > rand_x_b(*rng, dist_x_b);
        rectB.x = rand_x_b();

        boost::uniform_int<> dist_y_b(0 - patch_size/2, size - rectB.height - 1 - patch_size/2);
        boost::variate_generator<boost::mt19937&, boost::uniform_int<> > rand_y_b(*rng, dist_y_b);
        rectB.y = rand_y_b();

        assert(rectA.x >= 0 - patch_size/2 and rectB.x >= 0 - patch_size/2 and rectA.y >= 0 - patch_size/2 and rectB.y >= 0 - patch_size/2);
        assert(rectA.x + rectA.width <= patch_size/2 and rectA.y + rectA.height <= patch_size/2);
        assert(rectB.x + rectB.width <= patch_size/2 and rectB.y + rectB.height <= patch_size/2);
    }
...
struct SimplePixelFeature
{
    ...
    void generate(int patch_size, boost::mt19937* rng, int num_feature_channels = 0, float max_sub_patch_ratio = 1.0)
    {
        ...
        boost::uniform_int<> dist_size(0 - patch_size/2, patch_size - 1 - patch_size/2);
        boost::variate_generator<boost::mt19937&, boost::uniform_int<> > rand_size(*rng, dist_size);
